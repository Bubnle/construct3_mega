...
### 2)游戏概述
<a name="游戏概述"></a>
《Construct3 Mega》是一款结合了策略与即时动作的游戏，玩家将扮演一位强大的法师，面对不断涌现的哥布林，通过施放火球术击败敌人，保护自己并获得高分。并通过各种各样的道具来完成击败敌人的任务！

### 3)主要角色
<a name="主要角色"></a>
- **法师**:玩家控制的角色，能够施放火球术并指向鼠标位置进行攻击。
- **哥布林**:敌对角色，定期生成并向法师发起攻击。
- **boss**:敌对角色，定期生成攻击，并根据角色的位置选择不同的进攻策略。
- 
### 4)游戏机制
<a name="游戏机制"></a>
1. **目标**：尽可能多地击败哥布林，获得高分，同时避免被击中。
2. **操作方式**：
   - 玩家通过方向键控制法师移动。
   - 鼠标点击释放火球术，火球沿着鼠标方向移动。
     
### 5)可拓展的玩法
<a name="可拓展的玩法"></a>
1. **技能树系统**：
   - 玩家可以通过击败哥布林获取经验值，解锁新的技能，比如更强大的火球术、冰冻术（减缓哥布林移动）等。
...
3. **连击系统**：
   - 玩家可以通过连续击杀哥布林来获得连击奖励，提升得分，并在一定条件下解锁特殊效果，比如范围攻击火球。

4. **关卡挑战**：
   - 设定不同的关卡，每个关卡都有特定的任务，例如在规定时间内击杀一定数量的哥布林或保护特定的目标。

5. **Boss战**：
   - 在特定关卡中引入强大的Boss敌人，具有独特的攻击模式和更高的血量，击败Boss后会掉落稀有道具和大量分数。
     
6. **血量系统**：
   -哥布林拥有血量，受到攻击后减少，血量为零时销毁并释放特效。
   
7. **挑战性**：
   -定期生成哥布林，增加游戏的紧迫感，玩家需快速反应以保持生存
   
### 6)游戏流程
<a name="游戏流程"></a>
1. **开局设置**：玩家新建项目并选择背景与对象，设置哥布林和法师的初始位置。
2. **行为与事件**：
   - **行为**：为法师添加移动、镜头跟随等行为，确保流畅的操作体验。
   - **事件**：设计法师始终指向鼠标、施放火球、生成哥布林等事件逻辑。
3. **得分系统**：每击败一只哥布林，得分增加，并在界面上显示。
